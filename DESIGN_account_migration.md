...
## 10c. 能不能不重启客户端就完成迁移？（评估，2026-09-18）

**结论：核心可以，整体不建议现在就放开。** 逐项看：

| 迁移动作 | 客户端在跑时能改吗 | 依据 |
|---|---|---|
| `sessions.user_id`（**核心**） | ✅ **可以，且立刻生效** | 实测源码：`currentUserId()` 注释「惰性读取，保证账号切换后查询立刻生效」，SQL 每次实时拼 |
| `automations.owner_user_id` | ✅ 可以 | 同上，查询实时 |
| `memory/<uid>_memory.md` 改名 | ⚠️ 大概率可以 | 路径解析器是「纯字符串拼接，零 I/O」→ 每次调用重新拼路径，不缓存；**未实测** |
| `storage/user-<uid>-*/` 改名 | ⚠️ 大概率可以 | 同上；但 FileStorage 实例**按 scope 缓存**，改名后可能仍握着旧句柄 |
| `connectors/<uid>/` 改名 | ⚠️ 大概率可以 | 按需读文件；**未实测** |
| `settings.json` 键改名 | ✅ 可以 | 读-改-写，客户端按需读 |
| `account-snapshot.json` | ❌ **需要重启** | 注释明写「主进程**冷启动**关键路径上同步读盘」 |
| Chromium LevelDB | — 不需要改 | 实测里面是 sid→status 映射，key 是 sid 不是 uid |
| `status='working'` 的会话 | ❌ 不能迁 | 正在追加写 `.jsonl`，改了会撕裂 |

### 三个必须先验证的风险点

1. **`storage` 的 FileStorage 实例缓存**：源码注释「FileStorage 实例按 (scope | 'global') 缓存」。
   若改名后缓存仍指向旧路径，新账号会读到旧目录 → 表现为"设置串号"。
   **必须在测试账号上实测**：客户端运行中改目录名，再让客户端读一次。
2. **`account-snapshot.json` 会被内存态写回**：客户端在账号变化时会自己写这份快照
   （`ACCOUNT_SNAPSHOT_PERSIST_CHANNEL`）。也就是说 —— **我们可能根本不需要写它**，
   让客户端自己刷新即可。但要确认客户端在我们切号后**确实**会重写它。
3. **`memory` 是否有进程内缓存**：路径是零 I/O 拼的，但记忆内容本身可能被缓存过。

### 方案（若上面三点验证通过）

分两级，**默认仍是"要求客户端退出"**，只有验证通过后才把"免重启"暴露给用户：

```
迁移（客户端运行中）
├─ 跳过 account-snapshot.json      ← 交给客户端自己刷新
├─ 跳过 status='working' 的会话     ← 与现在一致
├─ 其余照常（sessions/automations/memory/storage/connectors/settings）
├─ 迁移完成后**等 1~2 秒**再校验     ← 给客户端 watcher 一点时间
└─ 结果里提示"若界面未刷新，请重启客户端"
```

**降级策略**：若校验发现客户端把归属写回去了（`user_id` 又变回旧值），
就退回"要求退出客户端"的路径并提示用户 —— 不要静默成功。

### 现在就能用的部分

`migrate(..., allow_client_running=True)` **已经实现**（自检和演练在用），
只是 UI 默认不开、也不给用户勾选。要放开只需把弹框里那个复选框接上，
**但建议先做完上面三点实测**。

> 判断依据：这三点里任何一条出错，用户看到的都是"迁移成功但数据错乱" ——
> 比直接要求重启一次糟糕得多。重启客户端的成本是几秒钟，误判的成本是数据。
