...
页面与 WorkBuddy 侧**共用同一份模板**，布局一致：**当前桌面端账号**（一行）+
**工具条** + **可用账号列表**。

- **查看当前桌面端账号**：单独一行，按 **uid** 反查它在 `tw_auth\` 里的素材文件，
  并显示在行尾的「账号库中：xxx」（找不到就写"不在账号库中"）。
- **可用账号**：**列表形式**，一行一个素材，横向三段 ——
  左「身份」（头像 / 昵称 / 文件名 / 登录态剩余）、中「积分明细」、右「操作」。
  **列表里不重复显示当前账号**。
- **一键续期**：对 `tw_auth` 全部素材各续期一次。Trae 侧**没有**积分与签到接口，
  所以「刷新积分」「一键签到」两个按钮和对应内容块**自动隐藏**（由 `UI_CONTEXT`
  里的 `CREDITS` / `CHECKIN` 置空控制）。
- **切换账号**：点击列表行上的"切换到该账号"，切换后尝试续期并刷新剩余天数。
- **添加账号**：展开"＋ 添加账号"，选择该账号的 storage.json 并填标识后保存。
- **删除账号**：点击列表行上的"删除"，确认后从 `tw_auth\` 移除素材。
...
| POST | `/api/refresh-all` | — | 一键续期：全部素材各续期一次 |

> Trae 侧**没有** `/api/credits` 与 `/api/checkin*`（WorkBuddy 才有的积分/签到接口），
> 对应按钮与内容块由模板按 `UI_CONTEXT` 自动隐藏。
...
| 端口 | 8765 | 8766 |
| 前端模板 | 共用 `ui_template.html`（按 `UI_CONTEXT` 渲染） | 同左 |
| 积分 / 签到 | 有（`/api/credits`、`/api/checkin*`） | **无**，按钮与内容块自动隐藏 |
| 账号条目的 `uid` | 有 | 有（**必须给**，前端靠它映射当前账号并去重） |
