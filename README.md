...
`.exp` 内部拆成两个 `nowrap` 片段（`.exp-ttl` / `.exp-when`），万一在极窄窗口下
仍要折行，会在两者之间**整体断开**，不会把 `·` 孤零零留在行尾。
...
现在把「进度条 + 已使用量」放进 `.ci-line` 同一排，并让 `.ci` 按内容宽度收缩：
...
两排的 `left` / `right` 边缘**完全对齐**（实测都是 300 → 600）。进度条用
`flex:1 1 60px` 吃掉数字之外的剩余宽度，所以条子既短、比例又能直接读。
`.ci { min-width:300px }` 是给条子的下限，避免数字太长时把条子挤没。
