...
身份列（`.who`）宽 **210px**，内部可用 169px。这个值是"刚好放得下
`workbuddy-<标识>.info` 一行"的最小值 —— 定小了会连带出下面两个问题：

- **`meta` 折成两行**，和下面的「登录态剩余」贴在一起，看起来像文字串行/错位。
  曾经这里还追加「· 桌面端正在使用」，但行首的 `[当前]` 标签已经说明同一件事 ——
  **重复信息把这一行挤到折行**，已去掉。
- **`exp` 折行后把 `·` 孤零零留在行尾**。现在拆成两个 `nowrap` 片段
  （`.exp-ttl` 登录态剩余 / `.exp-when` 日期），折行时在两者之间整体断开，不会出现悬空的 `·`。

两块都有 `line-height:1.4` 与独立的上边距，折行后也不会互相贴住。

**进度条**（`.ci-bar`）限了 `max-width:240px`，不再横贯整行 —— 与上方
「档位名 / 时间 / 已使用」那一行形成左对齐的视觉块，右侧留白。
