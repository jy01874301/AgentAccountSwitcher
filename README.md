...
- **已在运行、窗口收在系统托盘** → 先**再启动一次客户端**，走它自己的单实例唤醒
  （第二个实例把唤醒请求交给已运行的实例，由它自己 `win.show()`）—— **不重启**，1~2 秒。
  判据是「窗口从**隐藏**变成**可见**」：这是 Electron 自己做的动作，不会误判。
  **唤醒无效才回退重启客户端**（重开的窗口一定可交互）。
  ⚠️ 不要用外部 `ShowWindow` 唤出：那样唤出来的窗口 Win32 层**一切正常**
  （`visible`、`enabled`、不 `IsHungAppWindow`、`DWMWA_CLOAKED=0`，甚至
  `GetGUIThreadInfo` 显示 `hwndActive = hwndFocus = 它自己`），但**输入事件进不了渲染进程**
  —— 表现为「窗口有内容、鼠标键盘全无反应」。更麻烦的是这个故障态**从外部观测不到**，
  拿任何窗口属性做判据都会误判为"成功"。
- **没在运行** → 定位 exe 并 `DETACHED_PROCESS` 启动（不随本工具退出而结束），
  然后轮询等主窗口出现（最多 8 秒）再置前 —— 避免用户在这几秒里二次点击、插进初始化中途。

> 自检里用 `open_client(..., allow_restart=False)` 关掉"自动重启"这个副作用，
